...
from travella.services import account_limit_service, account_service, customer_profile_service, profile_image_service
...
def profile(request:HttpRequest) -> HttpResponse:
    try:
        ...
        booking_limit_count = account_limit_service.get_limit_counts(request.user.id, AccountLimit.Type.BOOKING)
        payment_limit_count = account_limit_service.get_limit_counts(request.user.id, AccountLimit.Type.PAYMENT)
        return render(request, view('profile'), {
            'profile_data': profile_data,
            'booking_status': booking_status,
            'tour_reminders': tour_reminders,
            'booking_limit_count': booking_limit_count,
            'payment_limit_count': payment_limit_count,
        })
